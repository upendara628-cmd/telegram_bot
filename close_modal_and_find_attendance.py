    ...
    if "api.campx.in" in url:
        ...
        try:
            if "application/json" in response.headers.get("content-type", ""):
                ...
                if "conducted" in body_str.lower() or "attended" in body_str.lower() or "attendance" in body_str.lower():
                    print(f"    [ATTENDANCE JSON FOUND!]: {url}")
                    print(f"    Data Snippet: {body_str[:300]}...")
...
def find():
    ...
    with sync_playwright() as p:
        ...
        logger.info(f"Navigating to: {attendance_url}")
        page.goto(attendance_url, timeout=60000)
        page.wait_for_timeout(5000)
        
        logger.info("Attempting to dismiss modal using selector...")
        try:
            ...
            if close_btn.count() > 0:
                ...
                logger.warning("dialog-close button not found.")
        except Exception as e:
            logger.warning(f"Error dismissing modal: {e}")
            
        page.wait_for_timeout(8000)
        
        # Write text after closing modal
        text = page.inner_text("body")
        with open("attendance_page_text_after_modal.txt", "w", encoding="utf-8") as f:
            f.write(text)
            
        page.screenshot(path="attendance_page_after_modal.png")
        context.close()
